...
public class Purchase {
    ...
    public static Purchase getPurchase_Purchase_Record(String id) {
        Purchase pr = null;
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from purchase1 where purchase_id= " + id + "");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                pr = new Purchase();
                pr.setPurchase_id(rs.getInt(1));
    ...
    public boolean delete() {
     ...
     public static int getMaxID_Purchase_Record() {
        ...
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from purchase1");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                id = rs.getInt(1);
            }
...
public static Object[][] view_Purchase_Records() {
        List<Purchase> l = Purchase.getPurchaseList_Purchase_Record();
        int no = 0;
        Object[][] v = new Object[l.size()][7];
        for (int i = 0; i < l.size(); i++) {
            no++;
...
public static Object[][] viewSearch_Purchase_Record(Date d) {
    ...
    public static List<Purchase> getPurchaseList_From_Search(String g) {
        List<Purchase> l = new ArrayList<Purchase>();
        try {
            System.out.println("Date:"+g);
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from purchase where purchasing_date like '"+g+"'");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                Purchase pr = new Purchase();
                pr.setPurchase_id(rs.getInt(1));
    ...
    public static List<Purchase> getPurchaseList_Purchase_Record() {
        List<Purchase> l = new ArrayList<Purchase>();
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from purchase1");
    ...
    public boolean insert_Purchase_Record() {
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("insert into purchase1 values(?,?,?,?,?,?,?)");
            pst.setInt(1, purchase_id);
...
public boolean UpdatePurchase_New_Purcahse() {
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("update purchase set prod_id=?,description=?,qty=qty+?,selling_unitprice=?,purchasing_price=?,purchasing_date=? where purchase_id=?");
            pst.setInt(7, purchase_id);
            pst.setInt(1, prod_id);
            pst.setString(2, description);
