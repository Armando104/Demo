...
public static Stockout getPurchase(String id) {
    ...
    public static int getSum_Stock_Out(String d) {
        int id = 0;
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select sum(amount) from stockout where recordingdate='"+d+"'");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                id = rs.getInt(1);
            }
