...
public class Expense {
 ...
 public static List<Expense> getList_Search_by_date1_date2_and_business(String n,String date1,String date2) {
        List<Expense> l = new ArrayList<Expense>();
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from expenses where business like '" + n + "' and recording_date between '"+date1+"' and '"+date2+"'");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                Expense pr = new Expense();
                pr.setExpense_id(rs.getInt(1));
    ...
    public static List<Expense> getList_Search_by_date_and_business(String n,String date) {
        List<Expense> l = new ArrayList<Expense>();
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from expenses where business like '" + n + "' and recording_date like '"+date+"'");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                Expense pr = new Expense();
                pr.setExpense_id(rs.getInt(1));
...
//        v[i][6]= l.get(i).getPurchasing_date();
  ...
  public static Object[][] viewExpenseNail_Search_by_date1_date2(String d1,String d2) {
        List<Expense> l = Expense.getList_Search_by_date1_date2_and_business("nail", d1,d2);
        int no = 0;
        Object[][] v = new Object[l.size()][5];
        for (int i = 0; i < l.size(); i++) {
          {
 ...
 public static Object[][] viewExpenseNail_Search_by_date(String d) {
        List<Expense> l = Expense.getList_Search_by_date_and_business("nail", d);
        int no = 0;
        Object[][] v = new Object[l.size()][5];
        for (int i = 0; i < l.size(); i++) {
          {
