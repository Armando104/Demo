...
public static Object[][]view_Search_ok(String d){
   List<Sale>l=Sale.getList(d);
   int no=0;int k=0;
   Object[][]v=new Object[l.size()][8];
     Calendar cal=new GregorianCalendar();
   for(int i=0;i<l.size();i++){
 ...
 public static Object[][] viewSearch(String d) {
     System.out.println("Date............."+d);
        List<Purchase> l = Purchase.getPurchaseList_From_Search(d);
...
public static List<Sale> getList(String date) {
        List<Sale> l = new ArrayList<Sale>();
        try {
            Connection con = DBClassConnector.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from sales where selling_date='"+date+"'");
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                Sale pr = new Sale();
                  pr.setSale_id(rs.getInt(1));
